...
class HAB(nn.Module):
    ...
    def __init__(self,
        ...
        # self.moe_layer = MoELayer(d_model=dim, d_ff=mlp_hidden_dim, num_experts=num_experts, dropout_rate=drop)
...
class ANABranch(nn.Module):
    ...
    def __init__(self, in_channels=1, base_channels=32):
        ...
        self.attention = AttentionEnhancedBlock(base_channels*2)

        self.up1 = UpBlock(base_channels*2, base_channels*2, base_channels)
        self.up2 = UpBlock(base_channels, base_channels, base_channels)

        self.out_conv = nn.Conv2d(base_channels, 1, kernel_size=1)

    def forward(self, x):
        skip1, x = self.down1(x)   # skip1: base_ch, x: base_ch, size/2
        skip2, x = self.down2(x)   # skip2: 2*base_ch, x: 2*base_ch, size/4
        x = self.attention(x)
        x = self.up1(x, skip2)     # skip2
        x = self.up2(x, skip1)     # skip1
        out = self.out_conv(x)
        return out
...
class NAGNET(nn.Module):
    ...
    def __init__(self,
        ...
        #------------------------- 0, noise level predict ------------------------- #

        # ------------------------- 1, shallow feature extraction ------------------------- #
        self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        self.conv_noise = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        ...
        self.noise_conv = nn.Conv2d(embed_dim, num_out_ch, 3, 1, 1)
        self.noise_conv2=nn.Conv2d(360, 180, kernel_size=1)
    ...
    def forward(self, x):
        ...
        noise = self.noise_predict(x)

        if self.upsampler == 'pixelshuffle':

            x = self.conv_first(x)
            x_features = self.conv_after_body(self.forward_features(x)) + x

            x = self.conv_before_upsample(x_features)
